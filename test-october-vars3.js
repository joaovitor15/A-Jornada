...
async function run() {
  ...
  const activeProfileId = profiles.find(p => p.tipo === 'pessoal')?.id;
  ...
  const cardDebts = {};
  userCards.forEach(c => {
      cardDebts[c.id] = { pastDespesas: 0, receitas: 0 };
  });
  ...
  console.log("totalUnpaidPast:", totalUnpaidPast);
  console.log("Final October Receitas:", saldoAntCalcAcumulado - historicProvisoes - historicRecorrentes - totalUnpaidPast);
}
run();
