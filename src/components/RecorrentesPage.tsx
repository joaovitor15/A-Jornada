...
export const RecorrentesPage = ({ activeProfileId }: RecorrentesPageProps) => {
  ...
  const isInvestimentosEnabled = true;
  ...
  return (
    <div className="p-6 md:p-8 max-w-7xl mx-auto space-y-6 pb-24">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex flex-col gap-[16px] w-full md:w-auto flex-1 justify-center items-center shrink-0">
          <div className="flex flex-row items-center gap-[12px] w-full md:w-auto justify-center">
            ...
            {dashboardPeriodo === 'mensal' && (
              <div className="relative flex-shrink-0" ref={dropdownRef}>
                ...
                >
                  <span>{MESES_COMPLETOS[mesAtual - 1]}</span>
        ...
        <div className="relative w-full md:w-auto shrink-0 select-none order-5">
          <button 
          ...
          >
            ...
            <span className="uppercase">Planejar Conta</span> 
            <ChevronDown size={14} className={`text-[#64748B] dark:text-[#94A3B8] transition-transform group-hover:scale-110 ml-1 ${isDropdownOpen ? 'rotate-180' : ''}`} />
          </button>
          ...
          <AnimatePresence>
            {isDropdownOpen && (
              <motion.div
                ...
                exit={{ opacity: 0, y: -10 }}
                className="absolute top-[100%] mt-2 right-0 bg-white dark:bg-[#0B0F19] rounded-2xl border border-[#E2E8F0] dark:border-[#1E293B] shadow-xl p-2 min-w-[220px] z-50 overflow-hidden"
              >
                {isInvestimentosEnabled && (
                  <>
                    <div 
                      ...
                      className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-amber-50 dark:hover:bg-amber-500/10 transition-colors cursor-pointer group"
                    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-8 md:mt-2">
        <div className="bg-gradient-to-br from-emerald-50/50 to-emerald-100/50 dark:from-emerald-950/20 dark:to-emerald-900/10 border border-emerald-500/20 shadow-[0_2px_12px_rgba(16,185,129,0.06)] p-5 rounded-3xl flex flex-col justify-center items-center h-full">
        ...
        <div className="bg-gradient-to-br from-rose-50/50 to-rose-100/50 dark:from-rose-950/20 dark:to-rose-900/10 border border-rose-500/20 shadow-[0_2px_12px_rgba(225,29,72,0.06)] p-5 rounded-3xl flex flex-col justify-center items-center h-full">
      ...
      {/* NATUREZA TABS */}
      <div className="flex gap-2 w-full lg:w-auto self-start flex-wrap mt-4">
        <button
              ...
              <div key={freqKey} className={`space-y-4 ${filtroNatureza === 'investimento' ? 'flex-1 flex flex-col' : ''}`}>
                {filtroNatureza !== 'investimento' && (
                  <div className="flex items-center gap-2 border-b border-[#E2E8F0] dark:border-[#1E293B] pb-2">
                    ...
                    <span className="bg-gradient-to-r from-[#2563EB] to-[#4F46E5] text-white text-[10px] font-bold px-2.5 py-0.5 rounded-full ml-2 shadow-sm">
                ...
                <div className={filtroNatureza === 'investimento' ? "grid grid-cols-1 gap-4 h-full flex-1" : "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4"}>
                  ...
                  <div 
                    key={item.id}
                    className={`bg-gradient-to-br from-white to-slate-50 dark:from-[#0B0F19] dark:to-[#0F172A] rounded-[20px] p-5 md:p-6 border border-[#E2E8F0] dark:border-[#1E293B] shadow-sm relative flex flex-col ${filtroNatureza === 'investimento' ? 'h-full min-h-[300px]' : ''}`}
                  >
                    {/* Header */}
                    <div className="flex flex-wrap justify-between items-start mb-4 gap-3">
                      ...
                      <div className="flex gap-2">
                        {filtroNatureza === 'investimento' ? (
                          <button 
                            className="flex items-center justify-center text-slate-400 hover:text-slate-600 dark:text-slate-500 dark:hover:text-slate-300 transition-colors bg-transparent hover:bg-black/5 dark:hover:bg-white/5 p-2 rounded-lg"
                            onClick={() => {
                    ...
                    {filtroNatureza === 'investimento' ? (() => {
                      ...
                      if (tagId && globalInvestments[tagId]) {
                         stats = globalInvestments[tagId];
                      } else if (recId && globalInvestments[recId]) {
                    ...
                    <div className="mt-auto space-y-2">
                      {dashboardPeriodo === 'anual' ? (
                        <div className="w-full py-3 px-4 rounded-xl text-sm font-bold bg-transparent border border-[#E2E8F0] dark:border-[#1E293B] text-slate-500 dark:text-slate-400 flex items-center justify-center gap-2 cursor-not-allowed">
                        ...
                        <button
                          ...
                          className="w-full py-3 px-4 rounded-xl text-sm font-bold bg-transparent border border-[#E2E8F0] dark:border-[#1E293B] text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors flex items-center justify-center gap-2"
                        >
                          <RotateCcw size={18} />
      ...
      {filtroNatureza === 'investimento' && listagemFiltrada.length > 0 && (() => {
         ...
         return (
            <div className="bg-gradient-to-br from-white to-slate-50 dark:from-[#0B0F19] dark:to-[#0F172A] rounded-[20px] p-5 md:p-6 border border-[#E2E8F0] dark:border-[#1E293B] shadow-sm relative flex flex-col w-full h-full min-h-[300px]">
               {/* Header */}
               <div className="flex flex-wrap justify-between items-start mb-4 gap-3">
                 <div className="flex flex-col gap-1 flex-1 min-w-[120px]">
                   <div className="flex items-center gap-2">
              ...
              <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input 
                        ...
                        className="w-full bg-slate-50 dark:bg-slate-900 border border-[#E2E8F0] dark:border-[#1E293B] rounded-xl py-2.5 pl-10 pr-4 text-base font-black text-slate-800 dark:text-white focus:outline-none focus:border-[#3B82F6]"
                      />
                    </div>
                ...
                {efetivarMostrarOpcoes && (
                  <div className="grid grid-cols-2 gap-3 mt-4">
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => {
                         ...
                         className="w-full bg-slate-50 dark:bg-slate-900 border border-[#E2E8F0] dark:border-[#1E293B] rounded-xl py-[9px] px-3 text-xs font-semibold text-slate-800 dark:text-white focus:outline-none focus:border-[#3B82F6]"
                      >
                    ...
                    {efetivarFormaPagamento === 'cartao_credito' && (
                      <div className="col-span-2 flex items-center gap-3 bg-[#F8FAFC] dark:bg-slate-800 p-2.5 rounded-xl border border-[#E2E8F0] dark:border-slate-700">
        ...
        {cancelFutureModal?.isOpen && (
          <div className="fixed inset-0 bg-black/50 z-[120] flex items-center justify-center p-4">
            <motion.div
              ...
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
              className="bg-white dark:bg-[#1E293B] rounded-3xl border border-[#E2E8F0] dark:border-[#334155] max-w-md w-full p-6 space-y-4 shadow-2xl"
            >
