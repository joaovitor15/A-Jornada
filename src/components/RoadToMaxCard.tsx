...
export const RoadToMaxCard: React.FC<RoadToMaxCardProps> = ({ card }) => {
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, y: 10 }}
      className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden flex flex-col sm:flex-row mb-4"
    >
      <div className="flex flex-col items-center justify-center p-4 bg-slate-50 w-full sm:w-48 border-b sm:border-b-0 sm:border-r border-slate-200 shrink-0">
        <div className="w-20 object-contain relative transition-transform hover:scale-105">
          ...
          />
          {card.playerLevel && (
            <div className="absolute -bottom-2 right-0 w-6 h-6 rounded-full bg-slate-800 text-white flex items-center justify-center text-xs font-black border-2 border-white shadow-sm z-10">
              {card.playerLevel}
            </div>
          )}
        </div>
        <div className="mt-3 text-center">
          ...
          {totalGoldToMax > 0 && (
            <div className="mt-2 text-xs font-semibold text-slate-500 flex items-center justify-center gap-1">
              Custo até o 16:{" "}
      ...
      <div className="flex-1 p-4 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 items-center content-center justify-items-center">
        {displayLevels.map((lvl) => {
          ...
          return (
            <div
              key={lvl}
              className={`w-full max-w-[140px] flex flex-col gap-1 border-2 rounded-lg p-2 ${state === "READY" ? "border-emerald-400 bg-emerald-50" : state === "ACTIVE" ? "border-blue-300 bg-blue-50/50" : "border-slate-100 bg-slate-50"}`}
            >
              ...
              <div className="flex-1 flex flex-col justify-center items-center">
                <div className="w-full relative h-[20px] bg-slate-200 rounded-md overflow-hidden text-[9px] font-black tracking-tight border border-slate-300/50 flex shadow-inner">
                  <div
                  ...
                  ></div>
                  <div
                    className={`absolute inset-0 flex items-center justify-center z-10 ${pct > 45 ? "text-white drop-shadow-[0_1px_1px_rgba(0,0,0,0.5)]" : "text-slate-700"}`}
                  >
              ...
              {cost.gold > 0 && (
                <div className="text-[10px] flex items-center justify-center gap-1 font-semibold text-slate-500 mt-1">
