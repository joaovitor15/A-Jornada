    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        <motion.div 
          ...
          className="fixed inset-0 bg-[#0F172A80] backdrop-blur-[4px]"
          onClick={onClose}
        />
        <motion.div 
          initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }}
          className="bg-white max-w-[460px] w-full rounded-[24px] shadow-[0_24px_48px_rgba(0,0,0,0.15)] relative z-10 flex flex-col max-h-[90vh] overflow-y-auto"
        >
          <div className="p-[20px]">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-[18px] font-[800] text-[#0F172A]">{recorrencia ? 'Editar Recorrência' : `Nova Recorrência (${tipo === 'receita' ? 'Receita' : 'Despesa'})`}</h2>
            ...
            <div className="space-y-[14px]">
              {/* Nome */}
              <div>
                <label className="block text-[12px] font-[700] text-[#64748B] uppercase tracking-wider mb-[6px]">Descrição</label>
                <input 
                  type="text" 
                  value={nome} 
              ...
              <div className="relative" ref={containerTagRef}>
                <label className="block text-[12px] font-[700] text-[#64748B] uppercase tracking-wider mb-[6px]">Tag</label>
                <div className="relative">
                  {tagSelecionada ? (
                    <div className={`inline-flex items-center gap-[6px] rounded-[100px] p-[6px_12px] text-[13px] font-[600] border-[1.5px] w-fit shadow-sm ${tipo === 'receita' ? 'bg-[#DCFCE7] text-[#16A34A] border-[#16A34A]' : 'bg-[#FEE2E2] text-[#EF4444] border-[#EF4444]'}`}>
              ...
              <div className="grid grid-cols-2 gap-[12px] items-end">
                <div>
                  ...
                  <input 
                    ...
                    readOnly
                    className={`w-full text-right border-[1.5px] border-[#E2E8F0] rounded-[14px] p-[10px_14px] text-[15px] font-[800] bg-[#F8FAFC] outline-none transition-all focus:border-[#2563EB] disabled:bg-[#F1F5F9] disabled:text-[#94A3B8] disabled:border-[#E2E8F0] ${!valorVariavel && tipo === 'receita' ? 'text-[#16A34A]' : ''} ${!valorVariavel && tipo === 'despesa' ? 'text-[#EF4444]' : ''}`} 
                  />
                </div>
                <div className="pb-3 flex items-center">
                  <label className="flex items-center gap-2 cursor-pointer group">
                    <div className={`w-5 h-5 rounded-[6px] border-[1.5px] flex items-center justify-center transition-colors ${valorVariavel ? 'bg-[#2563EB] border-[#2563EB]' : 'bg-[#F8FAFC] border-[#CBD5E1] group-hover:border-[#94A3B8]'}`}>
              ...
              <div className="grid grid-cols-2 gap-[12px] items-start">
                  <div>
                    <label className="block text-[12px] font-[700] text-[#64748B] uppercase tracking-wider mb-[6px]">Frequência</label>
                    <select value={frequencia} onChange={e => setFrequencia(e.target.value as any)} className="w-full border-[1.5px] border-[#E2E8F0] rounded-[14px] p-[10px_14px] text-[14px] font-[500] bg-[#F8FAFC] text-[#0F172A] outline-none cursor-pointer appearance-none transition-all focus:border-[#2563EB] focus:shadow-[0_0_0_3px_rgba(37,99,235,0.08)]">
                  ...
                  {frequencia === 'semanal' && (
                    <div>
                      <label className="block text-[12px] font-[700] text-[#64748B] uppercase tracking-wider mb-[6px]">Dia da Semana</label>
                      <select value={diaVencimento} onChange={e => setDiaVencimento(Number(e.target.value))} className="w-full border-[1.5px] border-[#E2E8F0] rounded-[14px] p-[10px_14px] text-[14px] font-[500] bg-[#F8FAFC] text-[#0F172A] outline-none cursor-pointer appearance-none transition-all focus:border-[#2563EB]">
                  ...
                  {frequencia === 'mensal' && (
                    <div>
                      <label className="block text-[12px] font-[700] text-[#64748B] uppercase tracking-wider mb-[6px]">Dia do Mês</label>
                      <input type="number" min="1" max="31" value={diaVencimento} onChange={e => setDiaVencimento(e.target.value ? Number(e.target.value) : '')} className="w-full border-[1.5px] border-[#E2E8F0] rounded-[14px] p-[10px_14px] text-[14px] font-[500] bg-[#F8FAFC] outline-none transition-all focus:border-[#2563EB]" placeholder="1 a 31" />
                    </div>
                  )}
              </div>
              ...
              {frequencia === 'anual' && (
                  <div className="grid grid-cols-2 gap-[12px] items-start">
                      ...
                      <div>
                        <label className="block text-[12px] font-[700] text-[#64748B] uppercase tracking-wider mb-[6px]">Mês</label>
                        <select value={mesVencimento} onChange={e => setMesVencimento(e.target.value ? Number(e.target.value) : '')} className="w-full border-[1.5px] border-[#E2E8F0] rounded-[14px] p-[10px_14px] text-[14px] font-[500] bg-[#F8FAFC] text-[#0F172A] outline-none cursor-pointer appearance-none transition-all focus:border-[#2563EB]">
                          <option value="">Selecione...</option>
              ...
              <div className="grid grid-cols-2 gap-[12px] items-end">
                {tipo === 'despesa' && cards.length > 0 ? (
                  <div>
                    <label className="block text-[12px] font-[700] text-[#64748B] uppercase tracking-wider mb-[6px]">Forma de Pagamento</label>
                    <select 
                      ...
                      className="w-full border-[1.5px] border-[#E2E8F0] rounded-[14px] p-[10px_14px] text-[14px] font-[500] bg-[#F8FAFC] text-[#0F172A] outline-none cursor-pointer appearance-none focus:border-[#2563EB]"
                    >
                ...
                <div className="pb-3 flex items-center">
                  <label className="flex items-center gap-2 cursor-pointer group">
                    <div className={`w-5 h-5 rounded-[6px] border-[1.5px] flex items-center justify-center transition-colors ${ativa ? 'bg-[#10B981] border-[#10B981]' : 'bg-[#F8FAFC] border-[#CBD5E1] group-hover:border-[#94A3B8]'}`}>
            ...
            <div className="mt-[24px] flex gap-[12px]">
              <button onClick={onClose} className="flex-1 bg-[#F1F5F9] text-[#64748B] font-[700] text-[14px] rounded-[14px] py-[12px] hover:bg-[#E2E8F0] transition-colors">Cancelar</button>
